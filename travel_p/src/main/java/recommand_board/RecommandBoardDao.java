		...
		public RecommandBoardDao() {
			...
			public RecommandBoardVo select(int num) {
				...
				try {
					...
					if (rs.next()) {// 첫 줄로 이동하여 데이터 있는지 확인
						...
						vo = new RecommandBoardVo(num2, writer, title, content, w_date);
					}
			...
			public ArrayList<RecommandBoardVo> selectAll() {
				...
				try {
					...
					while(rs.next()) {
						list.add(new RecommandBoardVo(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getDate(5)));
					}
				} catch (SQLException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
			...
			public ArrayList<RecommandBoardVo> selectReps(int parent) {
				Connection conn = dbconn.conn();
				String sql = "select * from board where parent=? order by num desc";
				ArrayList<RecommandBoardVo> list = new ArrayList<>();
				try {
